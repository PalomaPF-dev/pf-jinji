...
3. **異動申請書** — 指定帳票 **J-426(9)「異動申請書 ・ 組織名称追加変更申請書」**。起案→申請→承認→発令の一連。承認が揃うと人事マスターと組織図へ反映される。暦年連番（`J26-001`）で採番し、A4縦1枚の帳票として印刷できる。
4. **継続雇用申請書** — 指定帳票 **J-456「高齢者雇用・アルバイト契約満了に伴う継続雇用申請書」**。契約満了に伴う期間限定の継続雇用を申請する。年齢と実働時間は生年月日・勤務時間から自動計算する。
5. **人事考課** — 評価期（年度＋上期/下期）ごとの一次・二次評価。総合評価を入れて確定すると以後変更できない。項目マスターは設定画面で確認できる。
6. **基本給与** — 改定履歴を積み上げる履歴型。最新の有効行が現在の給与。取り消しは行を消さず無効化する。
7. **資格** — 保有資格と有効期限。期限までの残り日数を色分け表示し、期限接近を日次で点検する。
8. **ポータル連携** — この人事管理を「人」のマスターとして、所属・役職・人事情報をポータルへ送る。異動の発令が各業務アプリの部署・権限まで届く。
9. **設定** — 利用許可名簿・資格マスター・考課項目・監査ログ（責任者のみ）。
