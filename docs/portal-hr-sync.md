...
| ユーザーの所属部署・職場 | **人事管理** | 人事管理 → ポータル（コードで引き当てる。**部署は作らない**） |
| 部署・工場そのもの（コード・名称・親子）とアプリ割当 | **ポータル** | 連携しない（CSVで一覧だけ渡す） |
| 管理者権限（role='admin'） | **人事管理** | 承認者になっている人に付ける（外しはしない） |
| パスワード・ポータル管理（can_manage） | **ポータル** | 連携しない |
...
| `employees[].departmentCode` | | 所属する部・工場のコード（4桁）。ポータルに無ければ引き当てない |
| `employees[].workplaceCode` | | 所属する職場のコード（8桁）。ポータルに無ければ引き当てない |
| `employees[].isApprover` | | 誰かの承認者になっている人。ポータルで管理者権限（`role='admin'`）を付ける |

1回に送れるのは 2,000 名まで。人事管理側は 400 名ずつに分けて送る。

## ポータル側の挙動

1. `login_id` をキーに upsert する。既存行でも
   `role` / `can_manage` / `password_hash` には**触れない**
...
5. `departmentCode` / `workplaceCode` を**ポータルに同じコードの部署・職場が
   あるときだけ**引き当てる。**部署・職場そのものは作らない**。
   引き当てられなかったコードは `unknownDepartmentCodes` /
   `unknownWorkplaceCodes` で返す（部署CSVの取込漏れに気づけるように）
6. `isApprover` が立っている人に**管理者権限（`role='admin'`）を付ける**。
   ポータルは承認者に管理者権限を求める（`api/users.js` は管理者権限のユーザー
   しか承認者に指定させない）ため。**付けるだけで外さない**
   （ポータル側の判断で管理者にしている人が居るため）。
   ポータル管理（`can_manage`）には触れない。退職者には付けない

人事管理は400名ずつに分けて送るため、先の回では管理者本人がまだポータルに
居らず承認者が決まらないことがある。全員を送り終えてからもう一巡するので、
1回の「ポータルへ同期」で全員ぶんの承認者が入る。

### 部署・職場の一覧は CSV で渡す

ポータルに部署・職場が無いと**所属が引き当たらない**。人事管理の
`/settings` →「ポータル連携」から次の2つを落とし、ポータルの管理画面
「① 職場設定」の「CSVで一括設定」「職場CSVで一括設定」で取り込む。

| CSV | 中身 | 対応 |
|---|---|---|
| 部署CSV | `部署コード,種別,部署名,説明,並び順` | 人事の第2階層（部・工場） |
| 職場CSV | `部署コード,職場コード,職場名,並び順` | 人事の第3・第4階層 |

部署CSVに**アプリの列は入れない**。ポータルの取込は「アプリ列が無ければ
既存の割当を維持」するので、取り込み直してもアプリ割当が消えない。
「大口工場長」のように工場の枠へ統合される組織は職場として出さない。
