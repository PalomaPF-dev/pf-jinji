...
export default async function SettingsPage({
  ...
  const [admins, masters, items, logs, statusCounts, orgUnits] = await Promise.all([
    listAdmins(),
  ...
  // 初期化ボタンに「いま何件消えるか」を出す
  const employeeCount = Object.values(statusCounts).reduce((a, b) => a + b, 0);
  const orgUnitCount = orgUnits.length;
    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="space-y-8">
        ...
        <section>
          <div className="mb-3 flex items-start gap-2 rounded-xl border border-[#f0e2c8] bg-[#fdfaf3] p-4 text-xs text-[#a06a12]">
            ...
            <p>
              ...
              <strong>人事考課・基本給与・設定は、ポータル管理者（ポータル管理権限）だけ</strong>が扱えます。
              ポータル上の管理者（承認フロー上の管理者）は、その他の機能（社員台帳・組織図・申請書・資格）を
              自分の工場の範囲で扱えます。
              この名簿は、ポータルの権限が届かない状態でもアプリに入れる
              <strong>入室の控え</strong>です。名簿に載せても人事考課・基本給与・設定は開けません。
            </p>
          </div>
        ...
        <section className="space-y-5">
          ...
          <PortalSyncForm />
          <RestructureForm />
          <ResetHrDataForm employees={employeeCount} orgUnits={orgUnitCount} />
        </section>
