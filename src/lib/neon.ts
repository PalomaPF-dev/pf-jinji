...
const globalForPg = globalThis as unknown as {
  _jinjiPgPool?: Promise<PgPool>;
};

function getPool(url: string): Promise<PgPool> {
  if (!globalForPg._jinjiPgPool) {
    globalForPg._jinjiPgPool = import("pg").then(
      ({ Pool }) =>
        new Pool({
  ...
  return globalForPg._jinjiPgPool;
}
...
export function getSql(): SqlClient {
  ...
  if (shouldUsePgDriver(url)) return createPgClient(url);
